...
function cacheInterface () {
  ...
  */
  ...
  this.initCache = (logger, configJSON) => {
    return new Promise(
      resolve => {
  ...
  */
  ...
  this.flushCache = (logger, typeOfFlush, primaryEventData) => {
    if (typeOfFlush === 'event') {
      if (storagePolicyArchiveBy === 'time') {
        return Promise.resolve()
          .then(() => timeCacheInterface.flushSecondaryEventCache(logger, cache, primaryEventData))
          .catch(error => {
            throw error
          })
      } else {
        return Promise.resolve()
          .then(() => secondaryCacheInterface.flushSecondaryEventCache(logger, cache, primaryEventData))
          .catch(error => {
            throw error
          })
      }
      ...
      if (storagePolicyArchiveBy === 'time') {
        ...
        return Promise.resolve()
          .then(() => secondaryCacheInterface.flushCache(logger, cache))
          .catch(error => {
            throw error
          })
      }
