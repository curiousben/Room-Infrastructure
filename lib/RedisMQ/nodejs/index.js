...
function Subscriber (loggerConfigFilePath, redisMQConfigFilePath, configKey) {
    ...
    return new Promise(
      (resolve, reject) => {
        ...
        if (!(validRedisMQConfig['subscribers'].hasOwnProperty(configKey))) {
          reject(new InitializationError('\'' + this.id + '\' cannot be found in the subscribers section.'))
        }
        this.metaDataConfig = validRedisMQConfig['microservice.metadata']
  ...
  const transitListener = () => {
    return subscriberMethods.consumeMsg(this.client, this.subConfig['queue'], this.processingQueue)
      .then(resultObj => msgUtils.inflate({}, resultObj))
      .then(inflatedObj => {
        this.emit('MessageReady', inflatedObj)
      })
  ...
  *     queue or any other error is encountered in initialization.
  ...
  this.startConsuming = () => {
    ...
    return Promise.resolve()
      ...
      .then(processQueueLen => {
        if (processQueueLen === 0) {
          ...
          if ('persistent' == this.subType) {
            ...
            while (processQueueLen >= 0) {
              processPromises.push(subscriberMethods.consumeProcessElements(this.client, this.processingQueue, processQueueLen))
              processQueueLen--
            }
          } else {
      ...
      .then(() => {
        this.logger.info('The \'' + this.subType + '\' subscriber \'' + this.id + '\' has started to listen for messages on the queue \'' + this.subConfig['queue'] + '\'...', {'subscriber': this.id})
