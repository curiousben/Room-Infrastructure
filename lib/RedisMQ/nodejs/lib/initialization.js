...
// Checks to see if 'host', 'port', 'retry.time', and 'retry.attempts' are in the broker section
  ...
  for (var proKey in publishers) {
    if (!('topic' in publishers[proKey] && 'ack' in publishers[proKey])) {
      throw new InitializationError('----ERROR: The \'' + proKey + '\' section of the publishers section is missing one or many of the required publisher configurations. The publisher configurations are: \'topic\' and \'ack\' .')
    }
  }
  // Checks to see if 'topic', 'ack', and are in each individual consumer's configuration in the consumers section.
  const consumers = configObj['consumers']
  for (var conKey in consumers) {
    if (!('topic' in consumers[conKey] && 'ack' in consumers[conKey])) {
      throw new InitializationError('----ERROR: The \'' + conKey + '\' section of the consumers section is missing one or many of the required consumer configurations. The consumer configurations are: \'topic\' and \'ack\'.')
    }
  }
