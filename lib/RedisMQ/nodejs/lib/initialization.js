...
let checkRedisMQConfig = configObj => {
  ...
  if (!('queue' in errorPayload && 'ack' in errorPayload)) {
    throw new InitializationError('The errorPayload section in the microservice.metadata section is missing one or many of the required errorPayload configurations. The errorPayload configurations are: \'queue\' and \'ack\' .')
  }
  const errorMessage = microserviceMetadata['errorMessage']
  if (!('queue' in errorMessage && 'ack' in errorMessage)) {
    throw new InitializationError('The errorMessage section in the microservice.metadata section is missing one or many of the required errorMessage configurations. The errorMessage configurations are: \'queue\' and \'ack\' .')
  }
  ...
  for (var conKey in consumers) {
    if (!('queue' in consumers[conKey] && 'type' in consumers[conKey])) {
      throw new InitializationError('The \'' + conKey + '\' section of the consumers section is missing one or many of the required consumer configurations. The consumer configurations are: \'queue\' and \'type\'.')
    }
  }
