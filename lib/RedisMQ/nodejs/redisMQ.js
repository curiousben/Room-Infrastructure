    ...
    return Promise.resolve(payload)
      ...
      .then(onlyMetaData => {
        if (onlyMetaData) {
          ...
          .then(() => msgUtils.flatten('', payload))
          .then(flatMsg => publisherMethods.sendExistingDirect(this.client, this.metaDataConfig['component.id'] + '.process', this.metaDataConfig['errorPayload']['queue'], flatMsg['address'], flatMsg))
          .then(results => this.logger.error('Error payload has been sent to the errorPayload queue \'' + this.metaDataConfig['errorPayload']['queue'] + '\'', {'subscriber': this.id}))
          .catch(error => {
            throw error
          })
        } else {
      ...
      .then(result => msgUtils.errorMessage(this.metaDataConfig, error))
      .then(errMsg => msgUtils.createRedisMQMsg(errMsg))
      .then(newMsg => msgUtils.flatten('', newMsg))
      .then(flatNewMsg => publisherMethods.sendNewMsg(this.client, this.metaDataConfig['errorMessage']['queue'], flatNewMsg['address'], flatNewMsg))
      .then(results => this.logger.error('Error Message has been sent to the errorMessage queue \'' + this.metaDataConfig['errorMessage']['queue'] + '\'', {'subscriber': this.id}))
      .catch(error => {
        throw error
      })
  }
