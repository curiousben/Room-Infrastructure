        ...
        if (processQueueLen === 0) {
          ...
          while (processQueueLen >= 0) {
            processPromises.push(new Promise(
              (resolve, reject) => {
                console.log('HERE~~~~~~~~~~~~~~' + String([processingQueue, processQueueLen]))
                resolve(subscriberMethods.getProcessElements(this.client, processingQueue, processQueueLen))
              }
            ))
            processQueueLen--
          }
          ...
          //}
          //return clearProcess().then(res => console.log('Res: ' + res))
        }
      })
...
function Subscriber (configKey) {
  // Attaching EventEmitter to the subscriber
  EventEmitter.call(this)
